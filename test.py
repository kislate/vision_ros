...
def enhance_circle_edges(image, clahe_clip_limit=2.0, clahe_tile_grid_size=(8, 8),  gaussian_sigmaX=2):
    """增强圆形边缘"""
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    blurred = cv2.GaussianBlur(gray, (7, 7), gaussian_sigmaX)
...
# cv2.createTrackbar("gaussian_ksize", "TrackBars", 7, 20, empty)
cv2.createTrackbar("gaussian_sigmaX", "TrackBars", 2, 20, empty)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # 获取参数 (如果启用 TrackBars)
        if ENABLE_TRACKBARS:
            # gaussian_ksize = cv2.getTrackbarPos("gaussian_ksize", "TrackBars")
            gaussian_sigmaX = cv2.getTrackbarPos("gaussian_sigmaX", "TrackBars")
            ...
            enhanced_frame = enhance_circle_edges(frame, clahe_clip, (clahe_tile, clahe_tile), gaussian_sigmaX) # Also enhance with tunable parameters
        else:
            # 检测圆形 (使用默认参数)
